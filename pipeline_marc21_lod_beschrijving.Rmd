...
In dit experiment proberen we een selectie van de catalogus - 50 pamfletten uit - om te zetten naar Linked Open Data. We beginnen bij het programma [MarcEdit](https://marcedit.reeset.net/) en een [MARC bestand](https://solisservices.sharepoint.com/:u:/r/sites/LinkedDatabijMA2/Shared%20Documents/General/Test%20Van%20MarcEdit%20tot%20LOD/LOD-testset-Van_Someren.mrk?csf=1&web=1&e=aCn8DN), zonder externe identifiers. De benodigde bestanden zijn allemaal beschikbaar in [Github repository RubenSchalk/marctolod](https://github.com/RubenSchalk/marctolod).

## 2. Pipeline

De belangrijkste stappen van het experiment staan hieronder beschreven. De hele pipeline staat hieronder afgebeeld, samen met de corresponderende bestanden die voor elke stap nodig zijn (in rood).
...
In MarcEdit is het mogelijk om het MARC bestand te verrijken met externe identifiers zoals VIAF en FAST. Op deze manier zijn via Linked Data de pamfletten te verrijken met andere gegevens. In dit geval in hebben we ons gericht op persoonsnamen, maar het kan o.a. ook voor onderwerp, plaatsnamen en type werk.
...
**update 28-01-2022:** De opschoning in het bestand `marc_csv.R` is uitgebreid om ervoor te zorgen dat het conversieprogramma COW niet breekt over 1) variabelen die beginnen met nummers, 2) verkeerde encoding en 3) lege velden met spaties. Daarnaast worden de Worldcat en Worldcat UU URLs alvast in het R script gegenereerd, omdat in COW Jinja templating niet mogelijk is op datatype `xsd:anyURI` (m.a.w. COW kan geen URLs maken aan de hand van [joins](https://jinja.palletsprojects.com/en/2.11.x/templates/#join)). Dit alles is doorgevoerd in het script. Alle variabelen beginnen nu met `marc_` en de encoding is `UTF-8`.
...
De query [`image.rq`](https://druid.datalegend.net/RubenS/-/queries/images/5) laat al deels zien wat de mogelijkheden zijn. Hier worden van auteurs automatisch de geboortedatum en een afbeelding toegevoegd aan het zoekresultaat. Dit kan omdat de VIAF link is opgenomen in Wikidata (de gegevens achter Wikipedia). Via de VIAF link die we hebben toegevoegd aan onze metadata is nu dus **alle** informatie van deze auteur in Wikidata op te vragen - evenals alle data waarnaar verwezen wordt.
...
Het experiment is geslaagd omdat het gelukt is van 50 pamfletten Linked Open Data te maken met daarbij de toevoeging van VIAF links. Het is mogelijk om onze LOD automatisch te koppelen aan Wikidata en zo additionele gegevens op te halen. Tevens wordt de koppeling met onze Discovery omgeving hierbij behouden, waardoor de provenance gewaarborgd is. 

Het omzetten naar LOD bleek (vooral vanwege Ruben zijn achtergrond) geen al te grote uitdaging. Het grootste obstakel is de kwaliteit van de metadata. Verschillende systemen hebben ertoe geleid dat sommige metadata van mindere kwaliteit is dan andere en onvoldoende is gethesaureerd. In dat geval is het lastig om VIAF of andere links toe te voegen, waarmee de mogelijkheden van Linked Data niet optimaal benut worden. Koppeling met externe data is geen vereiste, maar maakt het veel makkelijker om gevonden te worden, links te maken met andere collecties en de metadata te contextualiseren. Het grootste knelpunt ligt daarom op het vlak van metadata, met name de standaardisering/ entity matching hiervan tegen thesauri.  
...
- UBU Linked Data omgeving (Triply?) om verder te experimenteren en resultaten toegankelijk te maken voor collega's.
